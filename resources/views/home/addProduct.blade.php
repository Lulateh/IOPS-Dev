...
<div class="mt-6 flex justify-center">
    <div class="bg-lilac bg-opacity-20 px-5 py-3 rounded-lg w-full sm:w-11/12 md:w-9/12 lg:w-8/12">
        <form action="{{ route('product.post') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <h2 class="font-Coda text-xl md:text-3xl font-bold text-center my-5">Agregar producto</h2>
    
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-16 gap-y-6 text-black font-medium">
                <div >
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="text" id="productName" name="productName" value="{{ old('productName') }}" required><br>
                        @error('productName')
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="text" id="brand" name="brand" value="{{ old('brand') }}" required><br>
                        @error('brand')
                            <div class="text-red-600 mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="text" id="price_income" name="price_income" value="{{ old('price_income') }}" required><br>
                        @error('price_income')
                            <div class="text-red-600 mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="text" id="price_sale" name="price_sale" value="{{ old('price_sale') }}" required><br>
                    @error('price_sale')
                        <div class="text-red-600 mt-1">{{ $message }}</div>
                    @enderror
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="text" id="location" name="location" value="{{ old('location') }}" required><br>
                        @error('location')
                            <div class="text-red-600 mt-1">{{ $message }}</div>
                        @enderror
                </div>
                <div >
                    <label for="details" class="font-Coda flex items-center text-wrap">
                        ...
                        Descripción </label> 
                    <input class="rounded-lg my-2 bg-white w-full h-16" type="text" id="details" name="details" value="{{old('details')}}" required><br>
                        @error('details')
                            <div class="text-red-600 mt-1">{{ $message }}</div>
                        @enderror
                    <label for ="expiration" class="font-Coda flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="mr-2" viewBox="0 0 16 16">
                    ...
                    <input class="rounded-lg my-2 bg-white w-full" type="date" id="expiration" name="expiration" value="{{ old('expiration') }}" required><br>
                    @error('expiration')
                        <div class="text-red-600 mt-1">{{ $message }}</div>
                    @enderror
                    <label for="img" class="font-Coda">Imagen </label> <br>
                    <input  type="file" id="img" name="img" required accept="image/*, .jpg, .jpeg, .png" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:hover:bg-blue-900 file:rounded-md file:border-0 file:bg-main-blue file:text-white"><br>
                    @error('img')
                        <div class="text-red-600 mt-1">{{ $message }}</div>
                    @enderror
                </div>
